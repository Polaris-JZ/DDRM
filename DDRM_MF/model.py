...
class PureMF(BasicModel):
    ...
    def bpr_loss(self, users, pos, neg):
        users_emb = self.user_emb(users.long())
        pos_emb   = self.item_emb(pos.long())
        neg_emb   = self.item_emb(neg.long())
...
class LightGCN(BasicModel):
    ...
    def __init_weight(self):
        ...
        # self.user_emb = initializer(torch.empty(self.num_users, self.latent_dim))
        # self.user_emb = Parameter(self.user_emb, requires_grad=True)
        # self.item_emb = initializer(torch.empty(self.num_users, self.latent_dim))
        # self.item_emb = Parameter(self.item_emb, requires_grad=True)
        ...
        if self.config['pretrain'] == 0:
            ...
            self.user_emb.weight.data.copy_(torch.from_numpy(self.config['user_emb']))
            self.item_emb.weight.data.copy_(torch.from_numpy(self.config['item_emb']))
    ...
    def getEmbedding(self, users, pos_items, neg_items, user_reverse_model, item_reverse_model, diff_model):
        ...
        # pos_emb_ego = self.embedding_t(pos_items)
        # neg_emb_ego = self.item_emb(neg_items)
    ...
    def predict(self, u_rep, i_rep):
        # pred = torch.sum(u_rep * i_rep, dim=-1)  # dim=-1在二维情况下等同于dim=1, 即按行求和
        u1, u2 = u_rep.chunk(2, 0)
        u11, u12 = u1.chunk(2, 0)
        u21, u22 = u2.chunk(2, 0)
        i1, i2 = i_rep.chunk(2, 0)
        i11, i12 = i1.chunk(2, 0)
        i21, i22 = i2.chunk(2, 0)
        pred1 = torch.sum(u11 * i11, dim=-1)
        pred2 = torch.sum(u12 * i12, dim=-1)
        pred3 = torch.sum(u21 * i21, dim=-1)
        pred4 = torch.sum(u22 * i22, dim=-1)
    ...
    def bpr_loss(self, users, pos, neg, user_reverse_model, item_reverse_model, diff_model):
        ...
        pos_pred = self.predict(users_emb, pos_emb)
        neg_pred = self.predict(users_emb, neg_emb)
        
        # loss = torch.nn.functional.softplus(neg_scores - pos_scores)
        loss = - F.logsigmoid(pos_pred - neg_pred)
        
        # return loss, reg_loss, reconstruct_loss, pos_scores
        return loss, reconstruct_loss, pos_pred
