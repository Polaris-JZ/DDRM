...
class PureMF(BasicModel):
    ...
    def __init_weight(self):
        self.users_feature = torch.nn.Embedding(
            num_embeddings=self.num_users, embedding_dim=self.latent_dim)
        self.items_feature = torch.nn.Embedding(
            num_embeddings=self.num_items, embedding_dim=self.latent_dim)
        print("using Normal distribution N(0,1) initialization for PureMF")
        
    def getUsersRating(self, users):
        users = users.long()
        users_emb = self.users_feature(users)
        items_emb = self.items_feature.weight
        scores = torch.matmul(users_emb, items_emb.t())
        return self.f(scores)
    
    def bpr_loss(self, users, pos, neg):
        users_emb = self.users_feature(users.long())
        pos_emb   = self.items_feature(pos.long())
        neg_emb   = self.items_feature(neg.long())
    ...
    def forward(self, users, items):
        ...
        items = items.long()
        users_emb = self.users_feature(users)
        items_emb = self.items_feature(items)
...
#             nn.init.xavier_uniform_(self.users_feature.weight, gain=1)
#             nn.init.xavier_uniform_(self.items_feature.weight, gain=1)
#             print('use xavier initilizer')
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
            nn.init.normal_(self.users_feature.weight, std=0.1)
            nn.init.normal_(self.items_feature.weight, std=0.1)
            print('use NORMAL distribution initilizer')
        else:
            self.users_feature.weight.data.copy_(torch.from_numpy(self.config['user_emb']))
            self.items_feature.weight.data.copy_(torch.from_numpy(self.config['item_emb']))
    ...
    def computer(self, diff_model, user_reverse_model, item_reverse_model, user, pos):
        ...
        """       
        users_emb = self.users_feature.weight
        items_emb = self.items_feature.weight
        ...
        #print(embs.size())
        light_out = torch.sum(embs, dim=1).squeeze(1)
    ...
    def getEmbedding(self, users, pos_items, neg_items, user_reverse_model, item_reverse_model, diff_model):
        ...
        users_emb_ego = self.users_feature(users)
        pos_emb_ego = self.items_feature(pos_items)
        neg_emb_ego = self.items_feature(neg_items)
